use anyhow::Result;
use chrono::{Local, Duration, Utc, DateTime};
...
pub fn show_statusline(read_stdin: bool) -> Result<()> {
    ...
    // Get current model from stdin if available
    let model_name = if read_stdin {
        let mut buffer = String::new();
        io::stdin().read_to_string(&mut buffer)?;
        
        if let Ok(hook_data) = serde_json::from_str::<HookInput>(&buffer) {
            Some(hook_data.model.display_name)
        } else {
            None
        }
    } else {
    ...
    // Calculate current session cost (last hour)
    let one_hour_ago = Utc::now() - Duration::hours(1);
    let recent_sessions = stats.sessions.iter()
        .filter(|s| s.last_activity > one_hour_ago)
        .map(|s| s.total_cost)
        .sum::<f64>();
    
    // Calculate burn rate (tokens per hour over last 3 hours)
    let three_hours_ago = Utc::now() - Duration::hours(3);
    let recent_sessions_3h: Vec<_> = stats.sessions.iter()
        .filter(|s| s.last_activity > three_hours_ago)
        .collect();
    
    let _tokens_3h: u64 = recent_sessions_3h.iter()
        .map(|s| s.tokens.total())
        .sum();
    
    let cost_3h: f64 = recent_sessions_3h.iter()
        .map(|s| s.total_cost)
        .sum();
    ...
    let block_start = DateTime::<Utc>::from_timestamp(block_start_hours * 3600, 0)
        .unwrap_or(now);
    
    let block_sessions: Vec<_> = stats.sessions.iter()
        .filter(|s| s.last_activity >= block_start)
        .collect();
    
    let block_cost: f64 = block_sessions.iter()
        .map(|s| s.total_cost)
        .sum();
    
    let block_tokens: u64 = block_sessions.iter()
        .map(|s| s.tokens.total())
        .sum();
    ...
    let model_display = model_name.as_deref().unwrap_or("Claude");
    
    // Compact format for statusline
    print!("🤖 {} | ", model_display);
    print!("💰 ${:.2} session / ${:.2} today / ", recent_sessions, today_cost);
    print!("${:.2} block ({:02}:{:02} left) | ", 
